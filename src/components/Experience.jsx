            ...
            <div className="space-y-8 md:space-y-16">
              {experiences.map((exp, index) => (
                ...
                >
                  ...
                  <motion.div 
                    className={`p-6 md:p-8 rounded-xl shadow-lg ${activeStep === index ? 
                  ...
                  >
                    <div className="flex items-center mb-3 md:mb-4">
                      <div className={`p-2 md:p-3 rounded-full mr-3 md:mr-4 ${activeStep === index ? 
                      ...
                      <div>
                        ...
                        <p className="text-xs md:text-sm opacity-80">{exp.date}</p>
                      </div>
                    </div>
                    <h4 className="font-medium mb-1 md:mb-2 text-sm md:text-base">{exp.company}</h4>
                    <p className="mb-3 md:mb-4 text-sm md:text-base">{exp.description}</p>
                    <div className="flex flex-wrap gap-1 md:gap-2">
                      {exp.skills.map((skill, i) => (
                        <span 
                          key={i}
                          className={`px-2 py-0.5 md:px-3 md:py-1 rounded-full text-xs font-medium ${activeStep === index ? 
                            'bg-white text-indigo-600' : 
                            'bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-300'}`}
                        >
                          {skill}
                  ...
                  {!isMobile && (
                    <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full bg-white dark:bg-gray-900 border-4 border-indigo-600 dark:border-indigo-500 z-10">
                      <span className={`text-base md:text-lg font-bold ${activeStep >= index ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'}`}>
                        {index + 1}
                      </span>
                    </div>
                  )}

                  {/* Desktop date on opposite side */}
                  {!isMobile && (
                    <div className="absolute right-1/2 top-0 mr-16 transform translate-y-1/4">
                      <motion.div
                        className={`px-4 py-2 md:px-6 md:py-3 rounded-lg ${activeStep === index ? 
                          'bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-300' : 
                          'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'} font-medium text-sm md:text-base`}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
